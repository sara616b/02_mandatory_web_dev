...
def _(tweet_id):
    ...
    try:
        ...
        if not tweet_text:
            ...
            return
        if len(request.forms.get("edit_tweet_text")) < 2:
            redirectPath = f"/tweets/edit/{tweet_id}?error=short"
            return
        if len(request.forms.get("edit_tweet_text")) > 250:
        ...
        current_image_id = request.forms.get("remove_image_id")
